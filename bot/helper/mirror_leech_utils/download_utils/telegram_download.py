...
class TelegramDownloadHelper:
    ...
    async def _download_progress(self, key, progress_dict, file_id):
        ...
        if size > 10 * 1024 * 1024:
            ...
            async def download_part(offset, limit):
                async with semaphore:
                    res = await self._download_chunk(message, offset, limit)
                    if res.is_error:
            ...
            if tasks:
                ...
                for res in results:
                    if isinstance(res, Exception):
                        LOGGER.error(f"Parallel download error: {res}")
                        await self._on_download_error(str(res))
                        return
    ...
    async def cancel_task(self):
        ...
        LOGGER.info(
            f"Cancelling download on user request: name: {self._listener.name} id: {self._gid}"
        )
